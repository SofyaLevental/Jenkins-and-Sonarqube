...
public class CalculatorMichaelSchmidt extends Dialog {
	...
	/*
	 * Create contents of the dialog, a display at the top and the various buttons.
	 * The Tooltip for each button explains its function.
	 * 
	...
	/*
	 * Create contents of the button bar, including a button to copy the display to
	 * the system clipboard and a close button.
	 * 
	 * @param parent
	 */
	@Override
...
//    newShell.setImage(KomoPlugin.getImageDescriptor(ApplicationImages.CALCULATOR_ICON).createImage());
	...
	/*
	 * (non-Javadoc) Method that copies contents of the display window to the
	 * clipboard when the Copy to Clipboard button is clicked.
	 * 
	...
	/*
	 * This method updates the value stored in memory. The value is cleared in the
	...
	/*
	 * This method converts the operator and display strings to double values and
	...
	private void updateCalc(char keyPressed) {
		...
		/*
		 * If there is no display value, the keystroke is deemed invalid and nothing is
		 * done.
		 */
		...
		/*
		 * If there is no operator value, only calculation key presses are considered
		 * valid. Check that the display value is valid and if so, move the display
		 * value to the operator. No calculation is done.
		 */
		...
		/*
		 * If '=' was pressed or result was invalid, reset pending calculation flag and
		 * operator value. Otherwise, set new calculation flag so calculations can be
		 * chained.
		 */
